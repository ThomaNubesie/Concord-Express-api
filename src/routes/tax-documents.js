...
router.get('/:country/:docType/:year', async (req, res) => {
  try {
    ...
    if (fs.existsSync(templatePath)) {
      ...
      // Footer
      drawLine(80);
      drawText('Concord Express Co Inc. · Ottawa, ON', 50, 62, { size: 9, color: rgb(0.6, 0.6, 0.6) });
      drawText('Manage your account & trips at concordexpress.ca', 50, 50, { size: 8, color: rgb(0.55, 0.55, 0.55) });
      drawText(`Document ID: ${country}-${docType}-${year}-${(userId || 'anon').slice(0, 8)}`, 50, 36, { size: 8, color: rgb(0.7, 0.7, 0.7) });
      drawText(new Date().toISOString().split('T')[0], 450, 36, { size: 8, color: rgb(0.7, 0.7, 0.7) });
    }

...
const { formatTimeInZone } = require('../lib/timezone');

// Route price bounds — synced with constants/cities.ts ROUTES array
const ROUTE_PRICES = {
  // ── CANADA ────────────────────────────────────────────────────────────────
  'ottawa-montreal':        { floor: 20,    ceiling: 45    },
  'montreal-ottawa':        { floor: 20,    ceiling: 45    },
  ...
  'cornwall-ottawa':        { floor: 12,    ceiling: 28    },
  'ottawa-peterborough':    { floor: 25,    ceiling: 55    },
  'peterborough-ottawa':    { floor: 25,    ceiling: 55    },
  'ottawa-quebec':          { floor: 22,    ceiling: 50    },
  'quebec-ottawa':          { floor: 22,    ceiling: 50    },
  'ottawa-chicoutimi':      { floor: 38,    ceiling: 80    },
  'chicoutimi-ottawa':      { floor: 38,    ceiling: 80    },
  'ottawa-moncton':         { floor: 60,    ceiling: 120   },
  'moncton-ottawa':         { floor: 60,    ceiling: 120   },
  'ottawa-fredericton':     { floor: 55,    ceiling: 110   },
  'fredericton-ottawa':     { floor: 55,    ceiling: 110   },
  'toronto-montreal':       { floor: 40,    ceiling: 85    },
  'montreal-toronto':       { floor: 40,    ceiling: 85    },
  'toronto-kingston':       { floor: 22,    ceiling: 48    },
  'kingston-toronto':       { floor: 22,    ceiling: 48    },
  'toronto-peterborough':   { floor: 15,    ceiling: 32    },
  'peterborough-toronto':   { floor: 15,    ceiling: 32    },
  'montreal-quebec':        { floor: 20,    ceiling: 45    },
  'quebec-montreal':        { floor: 20,    ceiling: 45    },
  'montreal-chicoutimi':    { floor: 24,    ceiling: 52    },
  'chicoutimi-montreal':    { floor: 24,    ceiling: 52    },
  'kingston-montreal':      { floor: 24,    ceiling: 52    },
  'montreal-kingston':      { floor: 24,    ceiling: 52    },
  'cornwall-montreal':      { floor: 12,    ceiling: 28    },
  'montreal-cornwall':      { floor: 12,    ceiling: 28    },
  'moncton-fredericton':    { floor: 15,    ceiling: 35    },
  'fredericton-moncton':    { floor: 15,    ceiling: 35    },
...
router.post('/:id/notify-checkin', verifyAuth, async (req, res) => {
  try {
    const { stop_name = 'your pickup stop', wait_mins = 5 } = req.body;
    const { data: trip } = await supabase
      .from('trips')
      .select('driver_id, from_city, to_city, bookings(passenger_id, status), driver:users!trips_driver_id_fkey(full_name)')
      .eq('id', req.params.id).single();
    ...
    const bookings = (trip.bookings || []).filter(b => ['confirmed','active'].includes(b.status));
    ...
    await Promise.all(bookings.map(b =>
      sendNotification({
        ...
        body:      `Your driver is at ${stop_name}. Please head to the pickup point now. You have ${wait_mins} minutes.`,
        relatedId: req.params.id,
      })
    ));

    res.json({ success: true, notified: bookings.length });
  } catch (err) {
...
router.post('/:id/running-late', verifyAuth, async (req, res) => {
  try {
    ...
    // Get driver's country for timezone
    const { data: driver } = await supabase.from('users').select('country').eq('id', req.userId).single();
    const newDepFmt = formatTimeInZone(newDep, driver?.country, trip.from_city);
